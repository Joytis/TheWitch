...
namespace Bird.Common.Data;

/// <summary>
/// Uploads run analytics for this mod's character to our Supabase backend via the game's official
...
/// upstream. We additionally gate on the mod's own Analytics Enabled config toggle.
///
/// Every character mod compiles its own copy of this class and subscribes separately; each copy
/// only uploads runs played as ITS character (<see cref="BirdModContext.Character"/>), so two
/// installed mods never double-report. All characters share ONE Supabase table; the promoted
/// <c>character</c> column (= mod id) tells them apart.
/// </summary>
internal static class RunAnalytics
{
    ...
    private static RunMetricsUploader s_uploader = null!;
    ...
    {
        s_ctx = ctx;
        s_uploader = new RunMetricsUploader(SupabaseEndpoint, SupabaseAnonKey, ctx.Logger);
    ...
    private static void OnMetricsUpload(SerializableRun run, bool isVictory, ulong localPlayerId)
    {
        ...
        {
            s_ctx.Logger.Warn($"Failed to build/upload {s_ctx.ModId} run analytics: {ex.Message}\n{ex.StackTrace}");
        }
    }

    private static void OnMetricsUploadInternal(SerializableRun run, bool isVictory, ulong localPlayerId)
    {
        if (s_ctx.AnalyticsEnabled != null && !s_ctx.AnalyticsEnabled())
        {
            s_ctx.Logger.Info($"Skipping {s_ctx.ModId} analytics upload, disabled in mod config.");
            return;
        }
        ...
        {
            s_ctx.Logger.Info($"Skipping {s_ctx.ModId} analytics upload: AutoSlay bot run.");
            return;
        }
        SerializablePlayer? localPlayer = run.Players.FirstOrDefault(p => p.NetId == localPlayerId);
        if (localPlayer?.CharacterId == null || localPlayer.CharacterId != s_ctx.Character().Id)
        {
            return; // Only runs played as this mod's character.
        }
    ...
    private static void Upload(SerializableRun run, bool isVictory, RunMetrics metrics)
    {
        ...
        // `character` (= mod id) is the per-character discriminator on the shared runs table;
        // see tools/analytics/migrations/001_character_column.sql.
        JsonObject row = new()
        {
            ["character"] = s_ctx.ModId,
        ...
        s_ctx.Logger.Info($"Uploading {s_ctx.ModId} run analytics...");
        s_uploader.Upload(row.ToJsonString(), $"{s_ctx.ModId} run analytics");
    }

    private static string GetModVersion() =>
        ModManager.GetLoadedMods().FirstOrDefault(m => m.manifest?.id == s_ctx.ModId)?.manifest?.version ?? "unknown";
