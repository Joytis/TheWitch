...
internal sealed class RunMetricsUploader
{
    ...
    public RunMetricsUploader(string endpoint, string apiKey, Logger logger)
    {
        _endpoint = endpoint;
        _apiKey = apiKey;
        _logger = logger;
    ...
    public void Upload(string json, string context)
    {
        if (!IsConfigured)
        {
            _logger.Info($"Analytics upload for '{context}' skipped: no endpoint configured.");
            return;
        }
    ...
    private async Task PostRequest(string json, string context)
    {
        ...
        {
            _logger.Warn($"Analytics upload for '{context}' failed due to network error: {ex.Message}");
            return;
        }

        if (response.IsSuccessStatusCode)
        {
            _logger.Info($"Analytics for '{context}' successfully uploaded.");
        }
        else
        {
            string body = await response.Content.ReadAsStringAsync();
            _logger.Warn($"Analytics upload for '{context}' failed with status {response.StatusCode}: {body}");
        }
    }
}
