...
.SYNOPSIS
    ...
    portraits dropped under <project>/<ModId>/images.
...
.DESCRIPTION
    ...
    This runs:  godot --headless --path <project dir> --import
...
$projDir = Join-Path $repoRoot $Char.ProjectDir
...
$projFile = Join-Path $projDir 'project.godot'
if (-not (Test-Path $projFile)) { throw "project.godot not found: $projFile" }
...
$proc = Start-Process -FilePath $godotExe `
    -ArgumentList @('--headless', '--path', "`"$projDir`"", '--import') `
...
Write-Host ""
Write-Host "Done. .import files under $($Char.ModId)/images:"
Get-ChildItem -Path (Join-Path $projDir "$($Char.ModId)/images") -Recurse -Filter '*.import' |
    ForEach-Object { Write-Host "  $($_.FullName.Substring($projDir.Length + 1))" }
