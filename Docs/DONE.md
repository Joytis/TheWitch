...
### 304. Bird Egg event vs Witch pets — investigation: already coexist, no fix needed
- **Done:** 2026-08-04 (claude) — traced gamedata. The event is **Byrdonis Nest** (`ByrdonisNest.IsAllowed`): blocked only when a player `HasEventPet()` — true iff they carry an `AddsPet` relic (`Byrdpip`, `PaelsLegion`) or a `ByrdonisEgg` card in deck. Witch familiar pets are combat-spawned monster visuals from `FamiliarPower` — no relic, no deck card — so they do **not** trip the gate: the Witch can reach the event, take the egg, hatch it (rest-site option → Byrdpip relic → `PlayerCmd.AddPet` each combat) while running familiars. Reverse direction also clean: `WitchPetClusterPatch` repositions/re-orders only `WitchPet` nodes and `WitchPetVisualsPatch` early-returns for non-WitchPet monsters, so the bird keeps the game's default pet layout. One cosmetic caveat: the game's line layout spaces ALL of a player's pets, then our patch pulls witch pets out to their markers — the bird may stand slightly off its solo spot. Nothing to prevent; no code change.
- **Verified:** code trace only. ⚠️ In-game sanity check: with familiars summoned + hatched Byrdpip, all pets render sanely (bird position acceptable).
- **Files:** none changed (read: `gamedata` ByrdonisNest/Byrdpip/Player.HasEventPet, `TheWitchCode/Monsters/WitchPetClusterPatch.cs`, `WitchPetVisualsPatch.cs`)

### 303. Shiny! — creates a Treasure (new sellable potion family)
- **Done:** 2026-08-04 (claude) — per user note. [Shiny.cs](../TheWitchCode/Cards/Familiar/Shiny.cs) is now **0e Skill token: Create a random Treasure** (upgraded: Big Treasure) — the old draw-pile fetch is gone (selectionScreenPrompt loc key removed). New [TreasurePotion.cs](../TheWitchCode/Potions/Treasures/TreasurePotion.cs): abstract `TreasurePotion` (Token rarity — registered but never a random drop/shop roll, per the pool-gating gotcha) modeled on base-game **FoulPotion**, the one other shopkeeper-throwable: `Usage AnyTime`, `TargetType` = AnyEnemy in combat / `TargetedNoCreature` outside, `PassesCustomUsabilityCheck` reuses `FoulPotion.GetFoulPotionMerchantTarget` (merchant present + inventory closed). In combat: single-target Unpowered damage. At shop: gold + the merchant "thank yous" sfx (skipped FoulPotion's slime-impact vfx + insult dialogue — this is a sale, not an insult). `SmallTreasurePotion` (5 dmg / 15g): Polished Bottlecap, Shiny Hairpin, Foreign Coin; `BigTreasurePotion` (7 dmg / 30g): Cracked Gemstone, Fancy Comb, Impeccable Silverware — identical mechanics, distinct ids → distinct image paths. Variant pick: `Rng.CombatPotionGeneration.NextItem` over the 3 canonicals (lazily resolved — ModelDb not populated at static-init time), procured via `Witch.ProducePotion` (stable). Card + belt tooltips: new TREASURE/BIG_TREASURE static hover tips. **Design calls:** FakeMerchant (event shop) NOT supported for selling — say the word if it should be; upgraded Shiny in-hand creates Big Treasures only (no mixed pool). ⚠️ **Missing art (6):** `potions/{polished_bottlecap,shiny_hairpin,foreign_coin,cracked_gemstone,fancy_comb,impeccable_silverware}.png` — placeholder fallback active; added to art-tracker assets.json.
- **Verified:** build 0/0; regen + art tracker run (TESTED cleared for Shiny!). ⚠️ In-game: play Shiny → random Treasure appears; throw one in combat (5/7 dmg); visit a shop and use it on the shopkeeper (gold + thank-you line); confirm it never drops randomly.
- **Files:** `TheWitchCode/Cards/Familiar/Shiny.cs`, `TheWitchCode/Potions/Treasures/TreasurePotion.cs` (new, 6 potions), `TheWitch/localization/eng/cards.json`, `TheWitch/localization/eng/potions.json`, `TheWitch/localization/eng/static_hover_tips.json`, `Docs/art-tracker/assets.json`, `Docs/card-data/cards.json`, `pages/art-tracker.html`

### 302. Hibernate — Strength next turn instead of Vigor next turn
- **Done:** 2026-08-04 (claude) — per user note (+ live clarification: strength arrives NEXT turn, not immediately). [Hibernate.cs](../TheWitchCode/Cards/Familiar/Hibernate.cs): keeps 15 Block; "+10 Vigor next turn" replaced with **+3 Strength next turn** via new [StrengthNextTurnPower.cs](../TheWitchCode/Powers/StrengthNextTurnPower.cs) (exact analog of VigorNextTurnPower: buff counter, `AfterPlayerTurnStart` applies StrengthPower then removes itself). Upgrade: +3 Block, +1 Strength. ⚠️ **Orphan surfaced:** `VigorNextTurnPower` (+ its loc keys) now has no user — left in place, say the word to cut it. ⚠️ Missing power icon: `strength_next_turn_power` (regen flagged).
- **Verified:** build 0/0; regen run (TESTED cleared). ⚠️ In-game: play Hibernate, Strength lands at next turn start and the tracker power expires.
- **Files:** `TheWitchCode/Cards/Familiar/Hibernate.cs`, `TheWitchCode/Powers/StrengthNextTurnPower.cs` (new), `TheWitch/localization/eng/cards.json`, `TheWitch/localization/eng/powers.json`, `Docs/card-data/cards.json`, `pages/art-tracker.html`

### 301. Salt and Ash — flat block + remove Unstable from a random potion
- **Done:** 2026-08-04 (claude) — per user note. [SaltAndAsh.cs](../TheWitchCode/Cards/SaltAndAsh.cs): now **9 (12+) Block** and removes Unstable from a random carried Unstable potion (`UnstablePotions.Unmark` on a `UnstableShuffle(Rng.CombatPotionGeneration)` pick among marked potions; no-op if none — that IS the "prefer Unstable" reading: only Unstable potions are candidates). Dropped the old "bonus Block if debuffed" mechanic + BonusBlock var. Added Unstable hover tip.
- **Verified:** build 0/0; regen run (TESTED cleared). ⚠️ In-game: with an Unstable potion on belt, play card → marker/vfx clears and potion survives combat end.
- **Files:** `TheWitchCode/Cards/SaltAndAsh.cs`, `TheWitch/localization/eng/cards.json`, `Docs/card-data/cards.json`, `pages/art-tracker.html`

### 300. Light the Candle & Hidden in Smoke — produce stable potions
- **Done:** 2026-07-30 (claude) — per user note. [LightTheCandle.cs](../TheWitchCode/Cards/LightTheCandle.cs) and [HiddenInSmokePower.cs](../TheWitchCode/Powers/HiddenInSmokePower.cs) now call `Witch.ProducePotion<T>(...)` with the default `PotionMode.Stable` instead of `PotionMode.Unstable`. Also dropped `UnstablePotions.UnstableHoverTip` from both cards' `ExtraHoverTips` (the Unstable keyword tip is misleading on a card that no longer makes one — Hidden in Smoke's tip lives on the card, not the power) and removed "Unstable" from all four loc strings (card + power description + smartDescription). The smoke potions themselves (`VialOfSmoke`, `PuffOfSmoke`) are unchanged.
- **Verified:** build 0/0; regen run (TESTED cleared for both). ⚠️ In-game: neither potion shows the Unstable marker, and neither is consumed by the Unstable teardown.
- **Files:** `TheWitchCode/Cards/LightTheCandle.cs`, `TheWitchCode/Cards/HiddenInSmoke.cs`, `TheWitchCode/Powers/HiddenInSmokePower.cs`, `TheWitch/localization/eng/cards.json`, `TheWitch/localization/eng/powers.json`, `Docs/card-data/cards.json`, `pages/art-tracker.html`

### 299. Rummage — upgrade discounts the retrieved card instead of fetching two
- **Done:** 2026-07-30 (claude) — per user note. [Rummage.cs](../TheWitchCode/Cards/Familiar/Rummage.cs) always fetches 1 card now (`OnUpgrade` no longer bumps `Cards`); upgraded, the retrieved card gets `EnergyCost.AddThisTurnOrUntilPlayed(-1, reduceOnly: true)`. Used the *OrUntilPlayed* variant on the game's own advice — `CardEnergyCost` documents that effects reading "this turn" almost always mean "this turn or until played", with plain `AddThisTurn` reserved for multi-play effects like Pinpoint. `reduceOnly` keeps it from ever raising a cost. The selection result had to be materialized to a list so the same instances can be discounted after `CardPileCmd.Add`.
- **Verified:** build 0/0; regen run. ⚠️ In-game: upgraded Rummage → fetched card shows a reduced cost that reverts at end of turn (or after one play).
- **Files:** `TheWitchCode/Cards/Familiar/Rummage.cs`, `TheWitch/localization/eng/cards.json`, `Docs/card-data/cards.json`, `pages/art-tracker.html`

### 298. Primal Form — doubles only the first 3 Attacks each turn
- **Done:** 2026-07-30 (claude) — per user note, revised same day from "first 3 attacks, once" to **per turn**. [PrimalFormPower.cs](../TheWitchCode/Powers/PrimalFormPower.cs): `Amount` is now the **per-turn allowance** (permanent, upgradeable) and a private `_duplicatedThisTurn` int tracks the spend, reset in `AfterPlayerTurnStart`. `ModifyCardPlayCount` adds +1 for the owner's Attacks while under the allowance; `AfterModifyingCardPlayCount` increments the counter — that hook only fires for models that actually changed the count and never during previews (`CardModel.GeneratePlayCount`), so non-Attacks and hover previews don't burn charges. Deliberately **not** a `PowerCmd.Decrement` counter (the earlier shape): decrement-to-zero would auto-remove a power that has to persist across turns, so the displayed counter stays at the allowance. The scratch field is a value type (no `DeepCloneFields` needed) and never has to survive a save — run saves hold no combat state and MP lockstep runs these hooks on every client. [PrimalForm.cs](../TheWitchCode/Cards/PrimalForm.cs) applies `PowerVar<PrimalFormPower>(3m)`; **upgrade is now +1 duplicated Attack** (was: remove Ethereal). ⚠️ **Design call to confirm:** taking the user's upgrade spec literally means the upgraded card *keeps* Ethereal — say so if it should do both.
- **Verified:** build 0/0; regen run (TESTED cleared). ⚠️ In-game: three Attacks double, the fourth doesn't, and the allowance refreshes next turn; power stays on the field.
- **Files:** `TheWitchCode/Powers/PrimalFormPower.cs`, `TheWitchCode/Cards/PrimalForm.cs`, `TheWitch/localization/eng/cards.json`, `TheWitch/localization/eng/powers.json`, `Docs/card-data/cards.json`, `pages/art-tracker.html`

### 297. Rip Soul — random non-healing potion instead of the curated table
- **Done:** 2026-07-30 (claude) — per user note. [RipSoul.cs](../TheWitchCode/Cards/RipSoul.cs) drops the hand-picked 7-potion `LootTable` and rolls `PotionCatalog.Random(PotionCatalog.Query(), Rng.CombatPotionGeneration)` — `Query()`'s defaults are exactly "any non-healing potion we can create": Witch+Shared pools only, Common/Uncommon/Rare (no Token/Event payload potions), healing excluded. No rarity restriction, so the result is no longer Rare-only. **Design call (user):** the potion stays **stable** — `PotionCmd.TryToProcure` as before, not `Witch.ProducePotion(..., PotionMode.Unstable)`; the stale "Unstable" wording had already been dropped from the loc, and "Rare" is now gone too.
- **Verified:** build 0/0; regen run (TESTED cleared). ⚠️ In-game: play it a few times, potions vary across rarities and never heal.
- **Files:** `TheWitchCode/Cards/RipSoul.cs`, `TheWitch/localization/eng/cards.json`, `Docs/card-data/cards.json`, `pages/art-tracker.html`

### 296. Pocket Rats — respec to a pure Rat generator, upgrade = +1 Rat
- **Done:** 2026-07-30 (claude) — [PocketRats.cs](../TheWitchCode/Cards/PocketRats.cs) per user note: now **1e Skill, Common, TargetType.Self — "Add 2 Rats into your Hand. Exhaust."**, upgrade **+1 Rat** (3 total). Dropped the 3-damage attack (and its `DamageVar` + `vfx_bite` hit fx + `cardPlay.Target` null-check). Reverts DONE 292's "upgrade creates Rats+" — created Rats are plain again, so `CreateInHand<Rats>` drops the `upgraded` arg and the hover tip is `FromCard<Rats>()`. Loc drops the damage line and the `{IfUpgraded:show:Rats+|Rats}` conditional. The optional `upgraded` param on `WitchFamiliarCard.CreateInHand` (added in 292) is left in place — still used elsewhere.
- **Verified:** build 0/0; `regen.js` run (TESTED auto-cleared for Pocket Rats). ⚠️ In-game: play it, 2 Rats in hand, no damage; upgraded gives 3.
- **Files:** `TheWitchCode/Cards/PocketRats.cs`, `TheWitch/localization/eng/cards.json`, `Docs/card-data/cards.json`, `pages/art-tracker.html`

### 295. MP softlock — dying to The Insatiable stalled the other player's turn
- **Done:** 2026-07-30 (claude) — [FamiliarPower.cs](../TheWitchCode/Powers/FamiliarPower.cs). Traced the full path: the Insatiable's kill is `SandpitPower.AfterRemoved`, which snapshots `AllAffectedCreatures` (player creature + **every entry in `Target.Pets`**) once, then walks it doing `GetCreatureNode(c).Visuals.Visible = false` **with no null check**, killing the player at index 0. Killing the player runs `RemoveAllPowersAfterDeath` → `FamiliarPower.AfterRemoved` → `CreatureCmd.Kill(pet)`; pets are `IsMonster`, so each kill calls `NCombatRoom.RemoveCreatureNode`. The outer loop then reaches those same pets, `GetCreatureNode` returns null, and the NRE unwinds out of the enemy turn — so `HandlePlayerDeath` never runs and the next `StartTurn()` never fires. Base-game pets don't hit this: Osty keeps its node (`ReattachPower.ShouldCreatureBeRemovedFromCombatAfterDeath` false), and nothing in vanilla kills a pet from a power's `AfterRemoved`. **Fix:** when the owner is dead, hide the pets' visuals instead of killing them (no node removal, no re-entrant death pipeline); the pet list is torn down with the combat anyway. Also added `Owner.IsDead` to the `SyncPets` guard so a dead owner can never spawn a fresh pet mid-teardown.
- **Verified:** build 0/0. ⚠️ Runtime-unverifiable — needs an MP playtest: Witch with ≥1 familiar out, die to The Insatiable, confirm the co-op partner's next turn starts.
- **Noted, not changed:** `WitchPetClusterPatch.cs:102` does `desired.InsertRange(desired.IndexOf(owner), allPets)` — `IndexOf` can return -1 if the owner node ever leaves the tree. Not on this path (it would NRE on `owner.GetParent()` first), left alone.
- **Files:** `TheWitchCode/Powers/FamiliarPower.cs`

### 294. Wormy offered by Discovery (combat card generation)
- **Done:** 2026-07-30 (claude) — [Wormy.cs](../TheWitchCode/Cards/Wormy.cs): added `CanBeGeneratedInCombat => false`. Root cause: base-game statuses live in the shared `StatusCardPool`, which no character's `CardPool` points at, so `Discovery` (`Owner.Character.CardPool.GetUnlockedCards`) never sees them. Wormy is a `WitchCard`, so it *is* in `WitchCardPool`, and `CardFactory.FilterForCombat` only strips Basic/Ancient/Event rarity — Status passes. Opting out per card matches base-game `Soot`/`FranticEscape` and is narrower than moving the card to a new pool. Card rewards and transforms were never affected (both rarity-gated).
- **Verified:** build 0/0. ⚠️ In-game: play Discovery with Wormy in the deck-pool — no Wormy in the 3 offers.
- **Files:** `TheWitchCode/Cards/Wormy.cs`
