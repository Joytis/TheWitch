...
### 301. Re-upgrade call sites to the STS2 beta API
- **Done:** 2026-07-31 (claude) — user switched back to the beta game dll; this re-applies (reverses) commit `d447fda` "Downgrade call sites to main-branch game API", plus two newer APIs it never covered. Signatures were read straight out of the installed `sts2.dll` via a `MetadataLoadContext` dump rather than from `gamedata/` (the decompile is main-branch and now stale). Changes: `.FromCard(this)` → `.FromCard(this, cardPlay)` (32 attack cards); every `CreatureCmd.Damage` overload gained a trailing `CardPlay?` (9 sites — cards pass `cardPlay`, powers/potions pass `null`); `ModifyDamageAdditive`/`ModifyDamageMultiplicative` overrides gained `CardPlay? cardPlay` ([HexPower.cs](../TheWitchCode/Powers/HexPower.cs), [EyeOfNewtPower.cs](../TheWitchCode/Powers/EyeOfNewtPower.cs)). Newer-than-`d447fda` breakages: `CombatManager.BeginCardOrPotionEffect(player)` now returns a `CombatId?` that `EndCardOrPotionEffect(combatId, player)` requires (and it's now awaitable) — [NeverendingPotionPower.cs](../TheWitchCode/Powers/NeverendingPotionPower.cs) threads it through; `Player.CanRemovePotions` was renamed `CanUseOrRemovePotions` — [PotionQuickUsePatch.cs](../TheWitchCode/Patches/PotionQuickUsePatch.cs).
- **Verified:** build 0/0 against the beta dll. ⚠️ Switching back to the live/main dll needs the reverse of all of this — re-apply `d447fda`'s shape and revert the two API renames.
- **Files:** 32 cards under `TheWitchCode/Cards/`, `TheWitchCode/Potions/{NoxiousBrew,PotionShapedPebble}.cs`, `TheWitchCode/Powers/{HexPower,EyeOfNewtPower,BramblesPower,MoonbeamPower,OverrunPower,RottingRootsPower,VolatileVaporsPower,NeverendingPotionPower}.cs`, `TheWitchCode/Patches/PotionQuickUsePatch.cs`
