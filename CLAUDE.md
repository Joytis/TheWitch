...
- **Editor junctions into `gamedata/`.** Repo-root `scenes/`, `materials/`, `shaders/`, `images/` are NTFS junctions → `gamedata/<same>` so the Godot editor resolves base-game `res://` paths referenced by mod scenes (they resolve from the game's pck at runtime either way). Gitignored — recreate on fresh clone with `New-Item -ItemType Junction <name> -Target gamedata\<name>`. They are excluded from export in `export_presets.cfg` (`exclude_filter`) — never remove those excludes, or proprietary game assets get packed into the mod `.pck` and shadow the game's own files. No `src/` junction: it would pull decompiled game `.cs` into compilation.
