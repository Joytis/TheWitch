...
- Handy: `list.UnstableShuffle(rng)` (in `MegaCrit.Sts2.Core.Extensions`) is the game's own in-place shuffle — use it instead of hand-rolling Fisher–Yates. And `BramblesPower` lives in namespace `MegaCrit.Sts2.Core.Models.Powers`, **not** `TheWicken.TheWickenCode.Powers` — a `using MegaCrit.Sts2.Core.Models.Powers;` is what fixes its CS0246.
