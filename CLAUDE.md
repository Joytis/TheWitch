...
- **"Make a potion of orientation X" cards** (the `OrientationBrewCard` subclasses: Wicked Brew = Offensive, Herbal Brew = Utility, Stony Brew = Defensive) just `PotionCatalog.Random(PotionCatalog.Query(orientation:, rarity:), rng)` — always `Random`, never `FirstOrDefault` (that yields the same potion every time).
...
- **Potions serialize as id + slot ONLY** (`SerializablePotion` in `gamedata/src/Core/Saves/Runs/`) — no per-potion state survives save/quit/resume or syncs in MP. Any stateful potion (state stored in the mutable instance's `DynamicVars`) silently resets to canonical on reload. Same caveat class as `FamiliarPower.GrantsUpgradedCards`. If persistence matters, use the sidecar-save pattern from git history (`CauldronSavePatch.cs`, cut with The Cauldron: user-dir JSON keyed by `PlayerRng.Seed` + slot; write on `Player.ToSerializable`, restore on private `Player.LoadPotions`) — still not MP-synced.
