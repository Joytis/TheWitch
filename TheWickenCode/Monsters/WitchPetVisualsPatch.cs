...
/// <summary>
/// Hijacks creature-visual creation for <see cref="WickenPet" />s: the pet reuses the base
...
public static class WickenPetVisualsPatch
{
    private static void Postfix(MonsterModel __instance, NCreatureVisuals __result)
    {
        if (__instance is not WickenPet pet || __result == null)
        {
            return;
        }
