...
A repeatable workflow for turning rough design notes into implemented, build-verified changes for TheWicken mod. Three docs under `Docs/`:
...
1. Turn it into a numbered `### N. <title>` item in `TODO.md` with: **Status** (`TODO` / `BLOCKED (reason)`), **Type**, a one-line **Rule/Decision**, the **Files** it'll touch (map design-doc names to real files first — search `TheWickenCode/` + `TheWicken/localization/eng/`), and crisp **Acceptance** criteria so the item is self-contained.
...
4. **Verify**: `dotnet build "<repo-root>/TheWicken.csproj"` must succeed — **0 errors** is the gate (build = deploy; there's no test suite). Run it from the **repo root**, never from `gamedata/` (that's the decompiled game's own `sts2.csproj`).
