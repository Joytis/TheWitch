...
///   --bird-cardtest    plays every card the mod defines (all pools): play from hand, draw it,
///                       exhaust it, discard it, then play Strike+Defend alongside it — the shape
///                       of Downfall's TestCode harness.
///   --bird-potiontest  procures + uses every mod potion (target auto-resolved by TargetType),
///                       then procures + discards it.
///   --bird-relictest   obtains EVERY mod relic on the player before the combat is built, then
///                       runs the card exercise for every card and the potion exercise for every
///                       potion with the whole relic set equipped.
///   --bird-menutest    main-menu screen sweep (MenuTest) — no combat; opens every menu
///                       screen + renders every mod hover tip.
///   --bird-testall     menu sweep, then cards, potions, relics in one process; one combined
///                       pass/fail summary + exit code.
/// Selection prompts auto-pick the first eligible cards.
/// "The mod" = the selected character's assembly (<see cref="BirdCharacter.Owns"/>).
/// </summary>
public static class CardTest
{
    ...
    {
        Mode.Potions => "[bird-potiontest]",
        Mode.Relics => "[bird-relictest]",
        Mode.Menu => MenuTest.Tag,
        Mode.All => "[bird-testall]",
        _ => "[bird-cardtest]",
    };

    /// <param name="priorTotal">Items already run by an earlier phase in this process (the menu sweep
    /// under --bird-testall); folded into the summary + exit code.</param>
    public static async Task RunAll(string seed, Mode mode = Mode.Cards, int priorTotal = 0, List<(string item, Exception ex)>? priorFailures = null)
    {
        ...
        try
        {
            ...
            if (!Godot.FileAccess.FileExists($"{BirdCharacter.ResPath}/localization/eng/cards.json"))
            {
                AutoSlayLog.Warn($"{tag} mod .pck not loaded (no {BirdCharacter.ResPath}/localization) — localization-dependent content will fail; run with Build=Publish");
            }

            // Every card the character's assembly defines, whatever pool it sits in: the main
            // pool, shared token pools (Witch familiars), and StatusCardPool strays (Ash, Wormy).
            List<CardModel> cards = ModelDb.AllCards
                .Where(BirdCharacter.Owns)
            ...
            foreach (Mode phase in phases)
            {
                ...
                async Task Run(string name, Func<CombatState, Player, Task> exercise)
                {
                    ...
                    try
                    {
                        (CombatState combat, Player player) = await NewCombat(character, seed, relics);
        ...
        finally
        {
            ...
            // launch (launch.ps1 -Headless) propagates the result as its exit code.
    ...
    private static async Task ExercisePotion(PotionModel model, CombatState combat, Player player)
    {
        ...
        // Regression (Witch's Bottled Message, duck-typed so this dev mod references no character
        // assembly): a bottled card with a captured cost must survive the run's PACKET serialization
        ...
        System.Reflection.MethodInfo? bottleMethod = potion.GetType().GetMethod("Bottle", [typeof(CardModel)]);
        if (bottleMethod != null && player.PlayerCombatState?.Hand.Cards.FirstOrDefault(c => !c.EnergyCost.CostsX) is { } toBottle)
        {
            toBottle.EnergyCost.SetUntilPlayed(0);
            bottleMethod.Invoke(potion, [toBottle]);
            object? bottledCost = potion.GetType().GetProperty("BottledCost")?.GetValue(potion);
            PacketWriter writer = new() { WarnOnGrow = false };
            writer.Write(RunManager.Instance.ToSave(null));
            AutoSlayLog.Info($"  bottled {toBottle.GetType().Name} (cost {bottledCost}); run packet {writer.BytePosition} bytes");
        }
