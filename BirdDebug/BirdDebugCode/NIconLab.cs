...
/// <summary>
/// Debug-only icon browser (launched via --bird-debug --bird-iconlab, see BirdDebug).
/// Shows every relic and potion — the selected character mod's content first (assembly rule,
/// <see cref="BirdCharacter.Owns"/>), base game underneath — rendered in the same composited
/// states the game uses, so mod art can be checked for parity against the
...
public partial class NIconLab : Control
{
    ...
    private sealed record Entry(string Name, Texture2D? Icon, Texture2D? Outline, Color PoolTint, bool IsMod);
    ...
    public override void _Ready()
    {
        ...
        int modRelics = relics.Count(e => e.IsMod);
        int modPotions = potions.Count(e => e.IsMod);
        int missing = relics.Concat(potions).Count(e => e.Outline == null);
        BirdDebug.Logger.Info($"Icon Lab ready: {relics.Count} relics ({modRelics} {BirdCharacter.Key}), "
            + $"{potions.Count} potions ({modPotions} {BirdCharacter.Key}), {missing} without an outline");
    }
    ...
    private Control BuildToolbar()
    {
        ...
        Label title = new() { Text = $"{BirdCharacter.Key} Icon Lab" };
    ...
    private Control BuildTab(List<Entry> entries)
    {
        ...
        AddSection(box, BirdCharacter.Key, entries.Where(e => e.IsMod).ToList());
        AddSection(box, "Base game", entries.Where(e => !e.IsMod).ToList());
        return scroll;
    }
    ...
    private Control BuildCell(Entry entry)
    {
        ...
        _cells.Add(new Cell(cell, icon, outline,
            $"{entry.Name} {(entry.IsMod ? BirdCharacter.Key : "base")}".ToLowerInvariant(),
    ...
    private static Texture2D? TryLoad(Func<Texture2D?> load, string id, string what)
    {
        ...
        {
            BirdDebug.Logger.Info($"Icon Lab: {id} has no {what} ({e.GetType().Name})");
            return null;
        }
    }
}
