...
/// <summary>
/// Debug-only FX browser scene (launched via -bird-debug -bird-fxlab, see BirdDebug).
...
/// mod vfx scenes under the selected character mod's res:// root). Each row: Play, and Copy (puts the exact string the
...
public partial class NFxLab : Control
{
    ...
    public override void _Ready()
    {
        ...
        BirdDebug.Logger.Info($"FX Lab ready: {sfx.Count} sfx, {vfx.Count} vfx");
    }
    ...
    private Control BuildToolbar()
    {
        ...
        Label title = new() { Text = $"{BirdCharacter.Key} FX Lab" };
        ...
        bool withOneShotToggle = false)
    {
        ...
        void AddRow(FxEntry entry, int depth)
        {
            ...
            copyBtn.Pressed += () =>
            {
                ...
                BirdDebug.Logger.Info($"FX Lab: copied '{entry.CopyText}'");
            };
    ...
    // Default tint for the spawn snippets (the Witch house purple; a constant, not per-character).
    private static readonly Color AccentColor = new("A803FF");
    ...
    private List<NodeVfxEntry> CollectNodeVfxEntries() => new()
    {
        ...
        new("NAdditiveOverlayVfx — screen flash", "[VfxColor]",
            (lab, tint) => NAdditiveOverlayVfx.Create(NearestVfxColor(tint ?? AccentColor,
        ...
        new("NGroundFireVfx — ground fire", "[VfxColor]",
            (lab, tint) => MirrorVfx<NGroundFireVfx>(NGroundFireVfx.AssetPaths.First(), lab.StageGround,
                ("_vfxColor", NearestVfxColor(tint ?? AccentColor,
        ...
        new("NSmokePuffVfx — smoke puff", "[VfxColor]",
            (lab, tint) => MirrorVfx<NSmokePuffVfx>(NSmokePuffVfx.AssetPaths.First(), lab.StageCenter,
                ("_color", IsPurplish(tint ?? AccentColor) ? NSmokePuffVfx.SmokePuffColor.Purple : NSmokePuffVfx.SmokePuffColor.Green))),
        new("NStabVfx — cursed needle", "[VfxColor]",
            (lab, tint) => MirrorVfx<NStabVfx>("res://scenes/vfx/stab_vfx.tscn", null,
                ("_vfxColor", NearestVfxColor(tint ?? AccentColor,
                    ...
                    (VfxColor.Blue, new Color("007bdd")), (VfxColor.Purple, AccentColor),
    ...
    private static readonly Dictionary<string, Func<Color?, string>> CopySnippets = new(StringComparer.Ordinal)
    {
        ...
        {
            string vc = SnappedVfxColor(tint, AccentColor,
                (VfxColor.Red, new Color("ff4020")), (VfxColor.Purple, AccentColor),
                (VfxColor.White, Colors.White), (VfxColor.Cyan, Colors.Cyan), (VfxColor.Gold, new Color("ffd700")));
        ...
        ["NAdditiveOverlayVfx — screen flash"] = tint =>
            AddLine($"NAdditiveOverlayVfx.Create({SnappedVfxColor(tint, AccentColor, (VfxColor.Green, new Color("00ff15")), (VfxColor.Purple, new Color("b300ff")), (VfxColor.Blue, Colors.Blue), (VfxColor.White, Colors.White), (VfxColor.Cyan, Colors.Cyan), (VfxColor.Gold, new Color("ffd700")))})")
        ...
        ["NGroundFireVfx — ground fire"] = tint =>
            AddLine($"NGroundFireVfx.Create(target, {SnappedVfxColor(tint, AccentColor, (VfxColor.Red, new Color("ff4020")), (VfxColor.Green, new Color("2fa800")), (VfxColor.Blue, new Color("0099cd")), (VfxColor.Purple, new Color("7821ff")), (VfxColor.White, Colors.White), (VfxColor.Black, Colors.Black))})")
        ...
        ["NSmokePuffVfx — smoke puff"] = tint =>
            AddLine($"NSmokePuffVfx.Create(target, NSmokePuffVfx.SmokePuffColor.{(IsPurplish(tint ?? AccentColor) ? "Purple" : "Green")})")
            + "\n// not preloaded — ExtraRunAssetPaths: NSmokePuffVfx.AssetPaths",
        ["NStabVfx — cursed needle"] = tint =>
            $".WithHitVfxNode(t => NStabVfx.Create(t, facingEnemies: true, {SnappedVfxColor(tint, AccentColor, (VfxColor.Red, new Color("ff4020")), (VfxColor.Green, new Color("00a52f")), (VfxColor.Blue, new Color("007bdd")), (VfxColor.Purple, AccentColor), (VfxColor.White, Colors.White), (VfxColor.Cyan, Colors.Cyan), (VfxColor.Gold, new Color("ffd700")))}))\n// not preloaded — ExtraRunAssetPaths: \"res://scenes/vfx/stab_vfx.tscn\"",
        ["NThinSliceVfx — thin slice"] = tint =>
            AddLine($"NThinSliceVfx.Create(target, {SnappedVfxColor(tint, Colors.Cyan, (VfxColor.Red, new Color("ff9900")), (VfxColor.Green, new Color("00a52f")), (VfxColor.Blue, new Color("007bdd")), (VfxColor.Purple, AccentColor), (VfxColor.White, Colors.White), (VfxColor.Cyan, Colors.Cyan))})"),
    ...
    {
        VfxColor color = NearestVfxColor(tint ?? AccentColor,
            (VfxColor.Red, new Color("ff4020")), (VfxColor.Purple, AccentColor),
            (VfxColor.White, Colors.White), (VfxColor.Cyan, Colors.Cyan), (VfxColor.Gold, new Color("ffd700")));
        ...
        foreach (NodeVfxEntry entry in CollectNodeVfxEntries())
        {
            ...
            copyBtn.Pressed += () =>
            {
                if (CopySnippets.TryGetValue(entry.Name, out Func<Color?, string>? snippet))
                {
                    ...
                    BirdDebug.Logger.Info($"FX Lab: copied spawn code for '{entry.Name}'");
                }
            };
            row.AddChild(copyBtn);
    ...
    private void SpawnNodeVfx(NodeVfxEntry entry)
    {
        try
        {
            ...
            if (node == null)
            {
                BirdDebug.Logger.Info($"FX Lab: '{entry.Name}' returned null (TestMode guard or missing context)");
                return;
            }
        ...
        {
            BirdDebug.Logger.Error($"FX Lab: node vfx '{entry.Name}' failed: {e}");
        }
    }
    ...
    private static void PlaySfx(FxEntry entry)
    {
        ...
        {
            BirdDebug.Logger.Error($"FX Lab: sfx '{entry.PlayKey}' failed: {e}");
        }
    }
    ...
    private void PlayVfx(FxEntry entry)
    {
        ...
        {
            BirdDebug.Logger.Error($"FX Lab: vfx '{entry.PlayKey}' failed: {e}");
        }
    }
    ...
    private static List<string> CollectFmodEventPaths()
    {
        ...
        {
            BirdDebug.Logger.Error($"FX Lab: FmodServer event enumeration failed: {e}");
        }

        if (events.Count == 0)
        {
            BirdDebug.Logger.Error("FX Lab: no FMOD events found via FmodServer");
        }
    ...
    private static List<FxEntry> CollectVfxEntries()
    {
        ...
        CollectFilesRecursive(BirdCharacter.ResPath, modScenes);
