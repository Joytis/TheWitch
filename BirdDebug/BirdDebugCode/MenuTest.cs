...
/// <summary>
/// Main-menu smoke test (--bird-menutest): at main-menu ready, opens every main-menu screen the
...
/// construction. Also renders the hover tips of every card / relic / potion / power of the selected
/// character's mod (assembly rule, <see cref="BirdCharacter.Owns"/>) and, in the Card Library, cycles
/// every pool filter (BaseLib adds the mod pool tab). Any exception is a logged failure; stray
/// [ERROR] lines are counted by launch.ps1's report.
/// </summary>
public static class MenuTest
{
    public const string Tag = "[bird-menutest]";
    ...
    public static async Task<(int total, List<(string item, Exception ex)> failures)> Run(NMainMenu menu)
    {
        ...
        AutoSlayLog.Action($"{Tag} opening every main-menu screen with {BirdCharacter.Key} ({character.Id}) loaded");
        ...
        await Step("CharacterSelect", async () =>
        {
            ...
            stack.Push(select);
            await Settle();
            NCharacterSelectButton? button = Descendants<NCharacterSelectButton>(select)
                .FirstOrDefault(b => b.Character.Id == character.Id);
            if (button == null)
            {
                throw new InvalidOperationException($"no {BirdCharacter.Key} button in character select");
            }
            if (button.IsLocked)
            {
                AutoSlayLog.Warn($"{Tag} {BirdCharacter.Key} button is locked; selecting it anyway");
            }
            button.Select();   // renders the character's portrait, starting relic + deck preview
        ...
        // --- Hover tips for every piece of the character mod's content --------------
        ...
        foreach (CardModel card in ModelDb.AllCards.Where(BirdCharacter.Owns).OrderBy(c => c.GetType().Name))
        {
            await Step($"HoverTips/Card/{card.GetType().Name}", () => ShowTips(menu, [HoverTipFactory.FromCard(card), .. card.HoverTips]));
        }
        foreach (RelicModel relic in ModelDb.AllRelics.Where(BirdCharacter.Owns).OrderBy(r => r.GetType().Name))
        {
            await Step($"HoverTips/Relic/{relic.GetType().Name}", () => ShowTips(menu, relic.HoverTips));
        }
        foreach (PotionModel potion in ModelDb.AllPotions.Where(BirdCharacter.Owns).OrderBy(p => p.GetType().Name))
        {
            await Step($"HoverTips/Potion/{potion.GetType().Name}", () => ShowTips(menu, potion.HoverTips));
        }
        foreach (PowerModel power in ModelDb.AllPowers.Where(BirdCharacter.Owns).OrderBy(p => p.GetType().Name))
        {
